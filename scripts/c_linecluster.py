...
#     cluster_list=[]
    ...
    for clster in range(0,cluster_size):
        c_cluster, vol_cluster, vol_prog_cluster = contingency_factor(n, 0,'DE',approach= app,c_start=c_start,c_end=c_end, N=int(N),cluster=clster)
        c.append(c_cluster)
...
def contingency_factor(
    ...
    if ct is not None:
        if cluster is not None:
            ...
            A_n1_ct_ign, b_ct_ign, x_ct_ign, y_ct_ign, ctr_ign = extract_country(n,i, A_n1,ct)
...
##################
                ...
                if np.all(p_line_norm.loc['now'] <= 1.0) != 1:
                    ...
                    S_new.S_modified[sub.branches().index[i_b]] = gl_opt* n.lines["s_nom"][line]
                    break
...
def calculate_initial_c_line(
    ...
    c_rob, v, vol, gl_line = find_c_robust_gl(n, i,ct,approach= 'robust',c_start= c_start,c_end= c_end, N=N)
    
    c_rob_uni, v, vol = contingency_factor(n, i,ct,approach= 'robust',c_start= c_start,c_end= c_end, N=N)
    ...
    for line in n.lines.index:
        if (n.lines.at[line, "bus0"] == sub.branches().bus0[i_b]) & (n.lines.at[line, "bus1"] == sub.branches().bus1[i_b]):
            C_r_line.C_of_line[sub.branches().index[i_b]] = c_rob * gl_line.gl_of_line[sub.branches().index[i_b]]
...
def calculate_c_line(
    ...
    C_r_line, c_rob_uni = calculate_initial_c_line(n, i, ct, c_start=c_start, c_end=c_end, N=N)
        ...
        for line in n.lines.index:
            if (n.lines.at[line, "bus0"] == sub.branches().bus0[i_b]) & (n.lines.at[line, "bus1"] == sub.branches().bus1[i_b]):
                if C_r_line.C_of_line[sub.branches().index[i_b]] > c_rob_uni:
                    ...
                    C_r_line.C_of_line[sub.branches().index[i_b]] = C_r_line.C_of_line[sub.branches().index[i_b]]
