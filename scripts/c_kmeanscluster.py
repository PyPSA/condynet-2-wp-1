"""Functions for determining approximate, robust and line-specific contingency factors based on k-means clustering."""
...
def cluster_country(n,
    ...
    in_cluster = []
    for j in range(0, y_km.size):
        if y_km[j] == cluster:
            in_cluster.append(j)
            A_cluster = A_ct[:, in_cluster]
    return A_cluster, max(y_km)
...
def contingency_factor_cluster(
    ...
    A_n0_ct, cluster_size = cluster_country(n, i,  A_n0, ct, cluster=0)
...
def contingency_factor(
    ...
    if ct is not None:
        if cluster is not None:
            A_n0_ct, cluster_size = cluster_country(n, i,  A_n0, ct, cluster)
            A_n1_ct, cluster_size = cluster_country(n, i,  A_n1, ct, cluster)
        else:
